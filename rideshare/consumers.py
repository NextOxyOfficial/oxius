...
class RideTrackingConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        ...
        allowed = ride.rider_id == user.id or (ride.assigned_driver_id and ride.assigned_driver.user_id == user.id) or user.is_superuser
        if not allowed:
        ...
        await self.accept()
        await self.send_json({"type": "connection.established", "ride_id": self.ride_id})
    ...
    async def receive_json(self, content, **kwargs):
        ...
        if event == "location.update":
            ride = await self._get_ride()
            if not ride or not ride.assigned_driver_id or ride.assigned_driver.user_id != user.id:
                await self.send_json({"type": "error", "message": "Only the assigned driver can send location updates."})
                return
    ...
    async def _get_ride(self):
        ...
        def _fetch():
            try:
                return Ride.objects.select_related("rider", "assigned_driver__user").get(id=self.ride_id)
    ...
    async def _save_location(self, payload):
        ...
        @database_sync_to_async
        def _store():
            ride = Ride.objects.select_related("assigned_driver__user").get(id=self.ride_id)
...
class DriverDispatchConsumer(AsyncJsonWebsocketConsumer):
    async def connect(self):
        ...
        if not driver_profile or driver_profile.approval_status != "approved" or not driver_profile.is_online:
            await self.close(code=4003)
            return
        ...
        await self.accept()
        await self.send_json({"type": "connection.established", "driver_id": str(user.id)})
