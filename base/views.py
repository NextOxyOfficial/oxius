...
class UserProductsListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return queryset
    def list(self, request, *args, **kwargs):
        """Override to include product statistics in response"""
        # Get the filtered queryset for pagination
        queryset = self.filter_queryset(self.get_queryset())
        
        # Only calculate statistics on first page and when no specific filters are applied
        # This ensures the statistics represent all user products, not just filtered ones
        page = request.query_params.get('page', '1')
        is_first_page = page == '1'
        has_filters = any([
            request.query_params.get('is_active'),
            request.query_params.get('has_stock'),
        ...
        if page_result is not None:
            ...
            # Add statistics to the response if calculated
            if stats:
