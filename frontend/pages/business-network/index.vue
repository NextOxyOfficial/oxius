<template>
  ...
  const searchResults = computed(() => {
    if (searchQuery.value.length > 0) {
      ...
      let mockResults = [`${searchQuery.value}`, `${searchQuery.value} trends`, `${searchQuery.value} examples`, `${searchQuery.value} best practices`, `${searchQuery.value} tips`];
  
      // If categories are selected, add them to the results
      if (selectedCategories.value.length > 0) {
        mockResults = mockResults.map(result => `${result} in ${selectedCategories.value.join(", ")}`);
      }
