<template>
  <UContainer>
    ...
    <div class="relative bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 dark:from-amber-950/30 dark:via-yellow-950/20 dark:to-orange-950/30 overflow-hidden">
      <!-- Floating Elements -->
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute top-20 left-10 w-4 h-4 bg-amber-400/20 rounded-full animate-pulse"></div>
        <div class="absolute top-40 right-20 w-6 h-6 bg-yellow-400/30 rounded-full animate-pulse" style="animation-delay: 0.5s"></div>
        <div class="absolute bottom-20 left-1/4 w-3 h-3 bg-orange-400/25 rounded-full animate-pulse" style="animation-delay: 1s"></div>
        <div class="absolute top-60 right-1/3 w-5 h-5 bg-amber-300/20 rounded-full animate-pulse" style="animation-delay: 1.5s"></div>
      </div>
      ...
      <div class="container mx-auto py-6 px-4 relative">
        <div class="text-center max-w-4xl mx-auto">
          ...
          <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
            <span class="text-gold-gradient-enhanced bg-clip-text text-transparent">
              {{ $t('gold_sponsors') }}
            </span>
          </h1>
          ...
          <p class="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mb-8 text-lg leading-relaxed">
            Discover exceptional businesses that support and contribute to our thriving community. 
            These gold sponsors represent excellence, innovation, and commitment to business growth.
          </p>
          
          <!-- Stats Bar -->
          <div class="flex flex-wrap justify-center gap-8 text-center">
            <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl px-6 py-4 border border-amber-200/50 dark:border-amber-800/50">
              <div class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ sponsors.length }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Active Sponsors</div>
            </div>
            <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl px-6 py-4 border border-amber-200/50 dark:border-amber-800/50">
              <div class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ totalViews }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Total Views</div>
            </div>
            <div class="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-xl px-6 py-4 border border-amber-200/50 dark:border-amber-800/50">
              <div class="text-2xl font-bold text-amber-600 dark:text-amber-400">Premium</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Partnership</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="container mx-auto py-8 px-4">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        <div class="flex items-center gap-4">
          <div class="relative">
            <UIcon name="i-heroicons-magnifying-glass" class="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
            <input 
              v-model="searchQuery"
              type="text" 
      ...
      <template v-if="isLoading">
        <div :class="viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8' : 'space-y-6'">
          <div v-for="i in 6" :key="i" class="animate-pulse">
            <div v-if="viewMode === 'grid'" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <div class="h-48 bg-gradient-to-r from-amber-100 to-yellow-100 dark:from-amber-900/30 dark:to-yellow-900/30"></div>
              <div class="p-6">
                <div class="flex items-center gap-4 mb-4">
                  <div class="w-16 h-16 bg-amber-200 dark:bg-amber-800 rounded-full"></div>
                  <div class="flex-1">
                    <div class="h-5 bg-amber-200 dark:bg-amber-800 rounded w-3/4 mb-2"></div>
                    <div class="h-4 bg-amber-100 dark:bg-amber-900 rounded w-1/2"></div>
                  </div>
                </div>
                <div class="space-y-2">
            ...
            <div v-else class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
              <div class="flex gap-6">
                <div class="w-24 h-24 bg-amber-200 dark:bg-amber-800 rounded-xl"></div>
                <div class="flex-1 space-y-3">
                  <div class="h-6 bg-amber-200 dark:bg-amber-800 rounded w-1/3"></div>
                  <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded"></div>
      ...
      <template v-else-if="error">
        <div class="text-center py-16">
          <div class="bg-white dark:bg-slate-800 rounded-2xl border border-red-200 dark:border-red-800 p-8 max-w-md mx-auto">
      ...
      <template v-else-if="filteredSponsors.length === 0">
        <div class="text-center py-16">
          <div class="bg-gradient-to-br from-amber-50 to-yellow-50 dark:from-amber-950/20 dark:to-yellow-950/20 rounded-2xl border border-amber-200 dark:border-amber-800 p-12 max-w-lg mx-auto">
            <div class="relative mb-6">
              <UIcon name="i-heroicons-star" class="w-20 h-20 mx-auto text-amber-400" />
              <div class="absolute inset-0 animate-ping">
                <UIcon name="i-heroicons-star" class="w-20 h-20 mx-auto text-amber-300 opacity-75" />
              </div>
            </div>
            <h3 class="text-2xl font-bold text-amber-600 dark:text-amber-400 mb-4">
              {{ searchQuery ? 'No sponsors found' : 'No gold sponsors yet' }}
            </h3>
            <p class="text-gray-600 dark:text-gray-400 mb-8 text-lg">
              {{ searchQuery ? 'Try adjusting your search terms.' : 'Be the first to become a gold sponsor and get featured prominently!' }}
            </p>
            <NuxtLink 
              v-if="!searchQuery"
              to="/contact"
      ...
      <template v-else>        <!-- Grid View -->
        <div v-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm hover:shadow-sm transition-all duration-500 group-hover:border-amber-300 dark:group-hover:border-amber-600">
              <!-- Banner Section -->
              <div class="relative h-48 bg-gradient-to-br from-amber-50 to-yellow-100 dark:from-amber-900/20 dark:to-yellow-900/30 overflow-hidden">
                <div v-if="sponsor.banners && sponsor.banners.length > 0" class="relative h-full">
                  <img 
                ...
                <div v-else class="flex items-center justify-center h-full">
                  <div class="text-center text-amber-600/50 dark:text-amber-400/50">
                    <UIcon name="i-heroicons-building-office" class="w-16 h-16 mx-auto mb-2" />
                    <p class="text-sm font-medium">{{ sponsor.name }}</p>
                  </div>
                </div>
                
                <!-- Premium Badge -->
                <div class="absolute top-4 right-4 bg-gradient-to-r from-amber-500 to-yellow-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">
                  <UIcon name="i-heroicons-star" class="w-3 h-3 inline mr-1" />
                  GOLD
                </div>
              </div>
              ...
              <div class="p-6">
                ...
                <div class="flex items-center gap-4 mb-4">
                  <div class="relative">
                    ...
                    />
                    <div class="absolute -bottom-1 -right-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-white rounded-full w-6 h-6 flex items-center justify-center shadow-sm z-20">
                      <UIcon name="i-heroicons-star" class="w-3 h-3" />
                    </div>
                  </div>
                  
                  <div class="flex-1 min-w-0">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-300 truncate">
                      {{ sponsor.name }}
                    </h3>
                ...
                <div class="space-y-2 mb-6">
                  <div v-if="sponsor.contact_email" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <UIcon name="i-heroicons-envelope" class="w-4 h-4 text-amber-500" />
                    <span class="truncate">{{ sponsor.contact_email }}</span>
                  </div>
                  <div v-if="sponsor.phone_number" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <UIcon name="i-heroicons-phone" class="w-4 h-4 text-amber-500" />
                    <span>{{ sponsor.phone_number }}</span>
                  </div>
                  <div v-if="sponsor.website" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <UIcon name="i-heroicons-globe-alt" class="w-4 h-4 text-amber-500" />
                    <span class="truncate">{{ sponsor.website.replace(/^https?:\/\//, '') }}</span>
                  </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                  <NuxtLink 
                    ...
                    :target="sponsor.profile_url.startsWith('http') ? '_blank' : '_self'"
        ...
        <div v-else class="space-y-6">
          <div 
            ...
            class="group"
          >
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 hover:shadow-sm transition-all duration-500 group-hover:border-amber-300 dark:group-hover:border-amber-600">
              <div class="flex flex-col md:flex-row gap-6">
                ...
                <div class="flex items-center gap-4 md:w-80 flex-shrink-0">
                  <div class="relative">
                    ...
                    />
                    <div class="absolute -bottom-2 -right-2 bg-gradient-to-r from-amber-500 to-yellow-500 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-sm z-20">
                      <UIcon name="i-heroicons-star" class="w-4 h-4" />
                    </div>
                  </div>
                  
                  <div class="flex-1 min-w-0">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors duration-300">
                      {{ sponsor.name }}
                    </h3>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                    <div v-if="sponsor.contact_email" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <UIcon name="i-heroicons-envelope" class="w-4 h-4 text-amber-500 flex-shrink-0" />
                      <span class="truncate">{{ sponsor.contact_email }}</span>
                    </div>
                    <div v-if="sponsor.phone_number" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <UIcon name="i-heroicons-phone" class="w-4 h-4 text-amber-500 flex-shrink-0" />
                      <span>{{ sponsor.phone_number }}</span>
                    </div>
                    <div v-if="sponsor.website" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <UIcon name="i-heroicons-globe-alt" class="w-4 h-4 text-amber-500 flex-shrink-0" />
                      <span class="truncate">{{ sponsor.website.replace(/^https?:\/\//, '') }}</span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <UIcon name="i-heroicons-calendar" class="w-4 h-4 text-amber-500 flex-shrink-0" />
                      <span>Since {{ new Date(sponsor.start_date).getFullYear() }}</span>
                    </div>
                  </div>
                  
                  <!-- Banners Preview -->
                  <div v-if="sponsor.banners && sponsor.banners.length > 0" class="flex gap-2 mb-4">
                    <div 
                      v-for="(banner, bannerIndex) in sponsor.banners.slice(0, 3)" 
                      :key="bannerIndex"
                  ...
                  <!-- Action Section -->
                <div class="flex flex-col justify-center gap-3 md:w-32 flex-shrink-0">
                  <NuxtLink 
...
const filteredSponsors = computed(() => {
  ...
  return sponsors.value.filter(sponsor => 
    ...
    (sponsor.business_description && sponsor.business_description.toLowerCase().includes(query)) ||
    (sponsor.contact_email && sponsor.contact_email.toLowerCase().includes(query))
  );
});

const totalViews = computed(() => {
  return sponsors.value.reduce((total, sponsor) => total + (sponsor.views || 0), 0);
});
...
.text-gold-gradient-premium {
  background: linear-gradient(135deg, #f59e0b, #fbbf24, #eab308, #fbbf24, #f59e0b);
...
.text-gold-gradient-enhanced {
  background: linear-gradient(135deg, #d97706, #f59e0b, #fbbf24, #eab308, #f59e0b, #d97706);
  background-size: 400% 400%;
  -webkit-background-clip: text;
  background-clip: text;
  color: transparent;
