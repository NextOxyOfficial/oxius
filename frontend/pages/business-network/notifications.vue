<template>
  ...
  >
    <!-- Header -->
    <header
      class="sticky sm:hidden top-0 z-10 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm"
    >
      <h1
        class="text-xl pl-6 font-semibold text-gray-800 bg-gradient-to-r from-blue-600 to-blue-600 bg-clip-text text-transparent"
      >
        Notifications
      </h1>
    ...
    <div class="max-w-2xl mx-auto pb-20">
      ...
      <!-- Load More Button -->
      <div
        v-if="filteredNotifications.length > 0 && hasMoreNotifications"
    ...
    <Teleport to="body">
      <div
        v-if="isFilterOpen"
        class="fixed inset-0 z-[9999] bg-black/50 flex items-end md:items-center justify-center p-4"
        @click="isFilterOpen = false"
      >
        <div
          class="bg-white rounded-t-xl md:rounded-xl w-full max-w-md overflow-hidden"
          @click.stop
        >
          <div
                  ...
                  <div
                    :class="[
                      ...
                      filter.value === 'all'
                        ...
                        : getFilterIconClass(filter.value),
                    ]"
                  >
                    <component
                      :is="getFilterIcon(filter.value)"
                      class="h-5 w-5"
                    />
                  </div>
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    const postTitle =
      type !== "follow"
        ...
        : null;
    const hasImage =
      ["like", "comment", "mention"].includes(type) && Math.random() > 0.5;
    ...
    return {
      ...
      postTitle,
      postImage: hasImage
        ? `/images/placeholder.jpg?height=100&width=100`
        : null,
      timestamp: new Date(
        Date.now() - id * 1000 * 60 * Math.floor(Math.random() * 60 * 24)
      ).toISOString(),
