        ...
        <div class="relative z-10 pt-20 sm:pt-24 md:pt-32 px-4 sm:px-6 md:px-8 pb-5 md:pb-6">
          ...
          <div class="flex flex-col md:flex-row md:items-end gap-4 md:gap-6 lg:gap-8">
            ...
            <div class="flex-shrink-0 -mt-20 pt-2 md:-mt-24 lg:-mt-28 relative mx-auto md:mx-0">
              <div class="h-24 w-24 md:h-28 md:w-28 lg:h-32 lg:w-32 rounded-xl bg-white p-1.5 shadow-sm">
          ...
          <div class="space-y-4">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
              <div class="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                <h3 class="font-medium text-gray-800 flex items-center">
                  <UIcon name="i-heroicons-rectangle-stack" class="h-4 w-4 mr-1.5 text-indigo-500" />
                  Categories
                </h3>
                <button 
                ...
                >
                  <span>Reset</span>
                  <UIcon name="i-heroicons-arrow-path" class="h-3 w-3 ml-1 group-hover:rotate-90 transition-transform duration-200" />
                </button>
              </div>
              <div class="p-4">
                <div class="space-y-1.5 max-h-60 overflow-y-auto pr-2 hide-scrollbar">
                  <button
                    @click="selectedCategory = null"
                  ...
                  >
                    <span class="flex items-center">
                      <UIcon name="i-heroicons-squares-2x2" class="h-4 w-4 mr-2 opacity-75" />
                      All Products
                    </span>
                    <span class="px-2 py-0.5 rounded-md text-xs bg-white shadow-sm border border-gray-100">
                      {{ products.length }}
                    </span>
                  </button>
                  ...
                  >
                    <span class="truncate mr-1 flex items-center">
                      <UIcon name="i-heroicons-tag" class="h-4 w-4 mr-2 opacity-75" />
                      {{ category.name }}
                    </span>
                    <span class="px-2 py-0.5 rounded-md text-xs" :class="selectedCategory === category.id ? 'bg-white shadow-sm border border-gray-100' : 'bg-gray-100'">
                      {{ getProductCountByCategory(category.id) }}
                    </span>
                  </button>
                </div>
                
                <div v-if="uniqueCategories.length === 0" class="py-4 text-center">
                  <div class="inline-flex items-center justify-center bg-gray-100 p-2.5 rounded-full mb-2">
                    <UIcon name="i-heroicons-tag" class="h-5 w-5 text-gray-400" />
                  </div>
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
              <div class="px-5 py-4 border-b border-gray-100 flex items-center">
                <UIcon name="i-heroicons-building-office-2" class="h-4 w-4 mr-1.5 text-indigo-500" />
                <h3 class="font-medium text-gray-800">Business Inquiries</h3>
              </div>
              
              <div class="px-5 py-4">
                <div class="bg-gradient-to-br from-indigo-50 to-blue-50 p-3.5 rounded-lg mb-4">
                  <p class="text-sm text-gray-700">Need large quantities or custom orders? Contact the seller directly for corporate deals and special pricing.</p>
                </div>
                ...
                <div class="space-y-3">
                  ...
                  >
                    <div class="h-10 w-10 bg-indigo-100 rounded-full flex items-center justify-center mr-3.5 group-hover:bg-indigo-200 transition-colors">
                      <UIcon name="i-heroicons-phone-arrow-up-right" class="h-5 w-5 text-indigo-600" />
                    </div>
                    <div class="flex-grow">
                      <p class="font-medium text-sm text-gray-800">Call Business Support</p>
                      <p class="text-xs text-gray-500 mt-0.5">{{ storeDetails.phone }}</p>
                    </div>
                    <UIcon name="i-heroicons-chevron-right" class="h-4 w-4 text-gray-400 group-hover:text-indigo-500 group-hover:translate-x-0.5 transition-all" />
                  </a>
                  
                  <!-- WhatsApp Contact -->
                  <a 
                    v-if="storeDetails.phone" 
                    :href="`https://wa.me/${cleanPhoneNumber(storeDetails.phone)}`" 
                    target="_blank"
                    rel="noopener noreferrer"
                    class="flex items-center p-3.5 border border-gray-100 rounded-lg transition-all duration-200 group hover:bg-gray-50 hover:border-gray-200 hover:-translate-y-0.5"
                  >
                    <div class="h-10 w-10 bg-green-100 rounded-full flex items-center justify-center mr-3.5 group-hover:bg-green-200 transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-600" viewBox="0 0 24 24" fill="currentColor">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M19.3929 4.63599C17.2799 2.51199 14.4219 1.33999 11.4069 1.33999C5.24294 1.33999 0.248932 6.33399 0.248932 12.5C0.248932 14.547 0.791943 16.5373 1.80994 18.291L0.166992 24L6.00594 22.38C7.69794 23.303 9.53094 23.79 11.4029 23.79H11.4069C17.5689 23.79 22.5629 18.796 22.5629 12.63C22.5629 9.60399 21.5059 6.75999 19.3929 4.63599ZM11.4069 21.85C9.74394 21.85 8.11694 21.383 6.70894 20.51L6.37894 20.315L2.92794 21.209L3.83194 17.836L3.61694 17.493C2.65894 16.028 2.15094 14.292 2.15094 12.5C2.15094 7.40199 6.30994 3.23999 11.4089 3.23999C13.9009 3.23999 16.2729 4.21299 18.0419 5.98999C19.8109 7.76699 20.6629 10.14 20.6609 12.63C20.6609 17.73 16.5039 21.85 11.4069 21.85ZM16.2789 14.98C16.0149 14.847 14.8229 14.262 14.5789 14.173C14.3349 14.085 14.1599 14.041 13.9829 14.304C13.8079 14.568 13.3429 15.107 13.1879 15.284C13.0329 15.46 12.8789 15.482 12.6149 15.349C11.2389 14.661 10.3169 14.115 9.39294 12.513C9.13994 12.083 9.71594 12.116 10.2539 11.039C10.3419 10.863 10.2979 10.708 10.2349 10.574C10.1729 10.44 9.61294 9.24799 9.39394 8.71999C9.17994 8.20399 8.96194 8.26799 8.79694 8.25899C8.64194 8.24999 8.46694 8.24999 8.29194 8.24999C8.11694 8.24999 7.82994 8.31199 7.58594 8.57599C7.34194 8.83999 6.70894 9.42599 6.70894 10.618C6.70894 11.809 7.60294 12.964 7.73494 13.139C7.86694 13.315 9.60694 16.0233 12.2949 17.063C14.2819 17.8109 14.9819 17.8359 15.8929 17.69C16.4399 17.6 17.3999 17.086 17.6189 16.47C17.8389 15.854 17.8389 15.326 17.7769 15.284C17.7149 15.242 17.5399 15.173 17.2759 15.041C17.0119 14.909 15.8199 14.324 15.5759 14.236C15.3319 14.147 15.1569 14.103 14.9819 14.368C14.8249 14.622 14.3819 15.112 14.2259 15.284C14.0709 15.455 13.9169 15.477 13.6539 15.349C13.3909 15.222 12.5739 14.95 11.6149 14.093C10.8659 13.429 10.3569 12.611 10.2019 12.347C10.0469 12.083 10.1859 11.941 10.3189 11.809C10.4379 11.69 10.5839 11.5 10.7389 11.345C10.8939 11.191 10.9379 11.08 11.0259 10.904C11.1139 10.728 11.0699 10.574 11.0069 10.44C10.9439 10.306 10.4509 9.11299 10.1879 8.57599L10.1879 8.57599Z"/>
                      </svg>
                    </div>
                    <div class="flex-grow">
                      <p class="font-medium text-sm text-gray-800">WhatsApp</p>
                      <p class="text-xs text-gray-500 mt-0.5">Message for quick response</p>
                    </div>
                    <UIcon name="i-heroicons-chevron-right" class="h-4 w-4 text-gray-400 group-hover:text-green-500 group-hover:translate-x-0.5 transition-all" />
                  </a>
                  
                  <!-- Email Contact -->
                  <a 
                    v-if="storeDetails.email" 
                    :href="`mailto:${storeDetails.email}?subject=Business%20Inquiry%20for%20${encodeURIComponent(storeDetails.store_name || '')}`" 
                    class="flex items-center p-3.5 border border-gray-100 rounded-lg transition-all duration-200 group hover:bg-gray-50 hover:border-gray-200 hover:-translate-y-0.5"
                  >
                    <div class="h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center mr-3.5 group-hover:bg-blue-200 transition-colors">
                      <UIcon name="i-heroicons-envelope" class="h-5 w-5 text-blue-600" />
                    </div>
                    <div class="flex-grow">
                      <p class="font-medium text-sm text-gray-800">Email Support</p>
                      <p class="text-xs text-gray-500 mt-0.5 truncate max-w-[140px]">{{ storeDetails.email }}</p>
                    </div>
                    <UIcon name="i-heroicons-chevron-right" class="h-4 w-4 text-gray-400 group-hover:text-blue-500 group-hover:translate-x-0.5 transition-all" />
                  </a>
                  
                  <!-- No Contact Info Available -->
                  <div v-if="!storeDetails.phone && !storeDetails.email" class="flex items-center justify-center p-5 bg-gray-50 rounded-lg">
                    <div class="text-center">
                      <div class="bg-gray-100 rounded-full p-3 inline-flex mb-3">
                        <UIcon name="i-heroicons-information-circle" class="h-6 w-6 text-gray-400" />
                      </div>
                      <p class="text-sm text-gray-600 mb-1">No contact information available</p>
                      <p class="text-xs text-gray-500">The store owner hasn't provided contact details yet</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <!-- Store Info Card -->
            <div v-if="storeDetails.store_description || storeDetails.store_address" class="bg-white rounded-xl shadow-sm overflow-hidden">
              <div class="px-5 py-4 border-b border-gray-100 flex items-center">
                <UIcon name="i-heroicons-information-circle" class="h-4 w-4 mr-1.5 text-indigo-500" />
                <h3 class="font-medium text-gray-800">About This Store</h3>
              </div>
              <div class="px-5 py-4">
                <div v-if="storeDetails.store_description" class="mb-4">
                  <h4 class="text-xs font-medium uppercase tracking-wider text-gray-500 mb-2">Description</h4>
                  <p class="text-sm text-gray-700 leading-relaxed">{{ storeDetails.store_description }}</p>
                </div>
                
                <div v-if="storeDetails.store_address" class="flex items-start">
                  <UIcon name="i-heroicons-map-pin" class="h-4 w-4 mt-0.5 mr-2 text-gray-400 flex-shrink-0" />
                  <div>
                    <h4 class="text-xs font-medium uppercase tracking-wider text-gray-500 mb-1">Location</h4>
                    <p class="text-sm text-gray-700">{{ storeDetails.store_address }}</p>
                  </div>
                </div>
              </div>
            </div>
...
function getProductCountByCategory(categoryId) {
  ...
  return products.value.filter(product => {
    ...
    if (product.category_details && product.category_details.id === categoryId) return true;
    return false;
  }).length;
}
