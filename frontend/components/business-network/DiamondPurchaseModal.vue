<template>
  <UModal 
    ...
    :ui="{ 
      ...
      container: 'max-h-[90vh] sm:max-h-[80vh]',
      wrapper: 'z-[9999]',
      base: 'sm:mt-16',
      overlay: {
  ...
  >
    ...
    <div class="rounded-xl overflow-hidden overflow-y-scroll">
      <!-- Header with gradient background -->
      <div class="p-4 bg-gradient-to-r from-pink-500 to-purple-500 text-white sticky top-0 z-10">
      ...
      <div class="bg-white dark:bg-slate-800">
        ...
        <div class="relative p-3 mx-4 mt-4 mb-3 rounded-xl overflow-hidden diamond-balance-card">
          ...
          <div class="relative flex justify-between items-center">
            <div>
              <div class="text-xs text-gray-600 dark:text-gray-400 mb-1">Available Diamonds</div>
              <div class="flex items-center">
                <span class="text-base font-bold mr-1">
                  <UIcon name="material-symbols:diamond-outline" class="size-4 text-pink-500" />
                  {{ user.user.diamond_balance }}</span>
                <UIcon name="i-heroicons-gem" class="h-4 w-4 text-pink-500" />
              </div>
            </div>
            <div>
              <div class="text-xs text-gray-600 dark:text-gray-400 mb-1">Account funds</div>
              <div class="text-base font-bold"><span class="text-lg font-bold">৳</span> {{ user.user.balance }}</div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'purchase'" class="p-4 pt-3">
          ...
          <div>
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Select Diamond Package</h4>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
              ...
              >
                <div class="absolute top-1 right-1 text-2xs font-semibold bg-pink-100 dark:bg-pink-900/50 text-pink-600 dark:text-pink-300 px-1 py-0.5 rounded-full">
                  <span class="text-xs font-medium">৳</span>{{ pkg.price }}
                </div>
                
                <UIcon name="i-heroicons-gem" class="h-6 w-6 text-pink-500 mb-0.5" />
                <div class="text-base font-bold text-gray-800 dark:text-gray-100">{{ pkg.amount }}</div>
                <div class="text-2xs text-gray-500">diamonds</div>
                
                <!-- Selected indicator -->
                <div v-if="selectedPackage === pkg.amount" class="absolute -top-1 -right-1 h-4 w-4 bg-gradient-to-br from-pink-500 to-purple-500 rounded-full flex items-center justify-center shadow-sm">
                  <UIcon name="i-heroicons-check" class="h-2.5 w-2.5 text-white" />
                </div>
              </button>
            </div>
            ...
            <div class="mb-3">
              <div class="text-xs text-gray-700 dark:text-gray-300 flex items-center gap-1 mb-1.5">
                <UIcon name="i-heroicons-information-circle" class="h-3.5 w-3.5 text-blue-500" />
                <span>10 diamonds = 1 BDT</span>
              </div>
              
              <div class="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg text-xs text-blue-800 dark:text-blue-200">
                You will be charged <span class="text-sm font-medium">৳</span>{{ calculateTotal }} from your account balance
              </div>
            </div>
        ...
        <div v-else class="p-4 pt-3">
          <!-- Loading state -->
          <div v-if="isLoadingHistory" class="py-12 flex flex-col items-center justify-center">
            <div class="relative h-8 w-8 mb-3">
              <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div>
              <div class="absolute top-0 left-0 rounded-full h-8 w-8 border-2 border-transparent border-t-pink-300 opacity-30"></div>
            </div>
            <p class="text-sm text-gray-500 dark:text-gray-400">Loading purchase history...</p>
          </div>

          <!-- Error state -->
          <div v-else-if="historyError" class="py-12 flex flex-col items-center justify-center">
            <UIcon name="i-heroicons-exclamation-circle" class="h-8 w-8 text-red-500 mb-3" />
            <p class="text-sm text-gray-700 dark:text-gray-300 mb-2">Failed to load history</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">{{ historyError }}</p>
            <UButton size="xs" color="pink" @click="loadPurchaseHistory">Try Again</UButton>
          </div>

          <!-- Empty state -->
          <div v-else-if="purchaseHistory.length === 0" class="py-12 flex flex-col items-center justify-center">
            <UIcon name="i-heroicons-document-text" class="h-8 w-8 text-gray-400 mb-3" />
            <p class="text-sm text-gray-700 dark:text-gray-300 mb-2">No purchase history</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">You haven't purchased any diamonds yet.</p>
            <UButton size="xs" color="pink" @click="activeTab = 'purchase'">Buy Diamonds</UButton>
          </div>

          <!-- History list -->
          <div v-else>
            <div class="mb-3 flex items-center justify-between">
              <h3 class="text-sm font-medium text-gray-800 dark:text-gray-200">Recent Transactions</h3>
              <UButton 
                size="xs" 
                variant="ghost" 
                color="gray" 
            ...
            <ul class="space-y-2">
              ...
              >
                <div class="flex justify-between items-center">
                  <div class="flex items-center">
                    <div class="bg-pink-100 dark:bg-pink-900/30 rounded-full p-1.5 mr-2">
                      <UIcon name="i-heroicons-gem" class="h-4 w-4 text-pink-500" />
                    </div>
                    <div>
            ...
            <div v-if="totalPages > 1" class="mt-4 flex justify-center">
              <div class="flex space-x-1">
                ...
                <div class="px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded-md text-xs">
...
const loadPurchaseHistory = async () => {
  ...
  try {
    ...
    historyError.value = error.response?.data?.error || 'Failed to load history'
  } finally {
