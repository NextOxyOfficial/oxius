<template>
  ...
  <!-- User Info - More compact with enhanced match highlighting -->
    <div class="flex-1">      
      ...
      <div v-if="searchQuery && searchQuery.trim() !== ''" class="flex flex-wrap gap-1 mb-1.5">
        <span 
          v-for="field in detectMatchedFields(user)" 
          :key="field"
          class="text-xs px-1.5 py-0.5 rounded-full"
          :class="{
            'bg-blue-100 text-blue-700 border border-blue-200': field === 'name',
            'bg-green-100 text-green-700 border border-green-200': field === 'username'
          }"
        >
      ...
      <NuxtLink :to="`/business-network/profile/${user.id}`" class="block">
        <h3 class="font-semibold text-gray-800 hover:text-blue-600 transition-colors" v-html="highlightMatchedText(user.first_name && user.last_name ? `${user.first_name} ${user.last_name}` : user.username, 'name')">
        </h3>
      </NuxtLink>
      <div class="flex flex-wrap items-center text-xs text-gray-500">
        <span v-if="user.username" class="mr-3" v-html="'@' + highlightMatchedText(user.username, 'username')"></span>
