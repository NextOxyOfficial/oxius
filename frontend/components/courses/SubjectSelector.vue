<template>  <div v-if="division" class="bg-white rounded-lg shadow-sm p-4 mt-3">
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="flex items-center text-xs text-gray-500">
        <span class="bg-gray-200 text-gray-700 rounded-full w-5 h-5 flex items-center justify-center mr-1">3</span>
      ...
      <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-md p-4 text-center">
        ...
        <button @click="loadSubjects" class="mt-2 text-xs text-blue-600 hover:underline">{{ $t('try_again') }}</button>
      </div>
      ...
      <!-- Subject grid -->
      <div v-else class="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        <div 
        ...
        >
          ...
          <div class="flex flex-col sm:flex-row items-center p-2 sm:p-3">
            ...
            <div class="p-1.5 rounded-lg mb-1 sm:mb-0 sm:mr-2" :class="subject.bgColor">
              <div v-if="subject.icon" v-html="subject.icon" class="w-5 h-5"></div>
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" :class="subject.iconColor" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
              </svg>
            </div>
            <div class="text-center sm:text-left">
              <h3 class="font-medium text-xs sm:text-sm">{{ subject.name }}</h3>
              <p class="text-xs text-gray-500 hidden sm:block">{{ subject.description }}</p>
            </div>
          </div>
            <div v-if="selectedSubject === subject.id" class="bg-blue-50 py-1 px-2 text-xs text-blue-600 border-t border-blue-200 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 sm:mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
