...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
    ...
    try {
      ...
      if (mounted) {
        if (result['auto_login'] == true && result['tokens'] != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.check_circle, color: Colors.white), const SizedBox(width: 8), Expanded(child: Text(result['message'] ?? 'Password reset successful!', style: AppFonts.roboto()))]), backgroundColor: const Color(0xFF10B981), behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))));
  ...
  Widget _buildHeader() => Container(padding: const EdgeInsets.all(16), child: Column(children: [Container(padding: const EdgeInsets.all(16), decoration: const BoxDecoration(color: Color(0xFFDEEAFF), shape: BoxShape.circle), child: Icon(Icons.lock_rounded, size: 32, color: Colors.blue.shade600)), const SizedBox(height: 16), Text('Reset Password', style: AppFonts.roboto(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.grey.shade900)), const SizedBox(height: 8), Text(_getStepDescription(), style: AppFonts.roboto(fontSize: 14, color: Colors.grey.shade600), textAlign: TextAlign.center)]));
  ...
  Widget _buildStepCircle(int step) { final isActive = _currentStep >= step; return Container(width: 32, height: 32, decoration: BoxDecoration(color: isActive ? Colors.blue.shade600 : Colors.grey.shade200, shape: BoxShape.circle), child: Center(child: Text('$step', style: AppFonts.roboto(fontSize: 14, fontWeight: FontWeight.w600, color: isActive ? Colors.white : Colors.grey.shade600)))); }
  ...
  Widget _buildMessageBox(String message, {required bool isError}) => Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: isError ? const Color(0xFFFEF2F2) : const Color(0xFFF0FDF4), borderRadius: BorderRadius.circular(8), border: Border.all(color: isError ? const Color(0xFFFCA5A5) : const Color(0xFF86EFAC))), child: Row(children: [Icon(isError ? Icons.error_outline : Icons.check_circle_outline, color: isError ? const Color(0xFFDC2626) : const Color(0xFF16A34A), size: 20), const SizedBox(width: 12), Expanded(child: Text(message, style: AppFonts.roboto(color: isError ? const Color(0xFF991B1B) : const Color(0xFF166534), fontSize: 13)))]));
  Widget _buildStep1() => Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Text('Phone Number', style: AppFonts.roboto(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade700)), const SizedBox(height: 8), TextField(controller: _phoneController, keyboardType: TextInputType.phone, style: AppFonts.roboto(fontSize: 14), decoration: InputDecoration(hintText: '01XXXXXXXXX', hintStyle: AppFonts.roboto(color: Colors.grey.shade400), prefixIcon: Icon(Icons.phone_rounded, color: Colors.grey.shade400), filled: true, fillColor: Colors.grey.shade50, border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.blue.shade500, width: 2)), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12))), const SizedBox(height: 24), SizedBox(height: 48, child: ElevatedButton(onPressed: _isLoading ? null : _handleSendOtp, style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade600, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), disabledBackgroundColor: Colors.grey.shade300), child: _isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(Colors.white))) : Text('Send Reset Code', style: AppFonts.roboto(fontSize: 15, fontWeight: FontWeight.w600))))]);
  Widget _buildStep2() => Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Text('We sent a 6-digit code to $_maskedContact', style: AppFonts.roboto(fontSize: 13, color: Colors.grey.shade600), textAlign: TextAlign.center), const SizedBox(height: 20), Text('Verification Code', style: AppFonts.roboto(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade700)), const SizedBox(height: 8), TextField(controller: _otpController, keyboardType: TextInputType.number, maxLength: 6, textAlign: TextAlign.center, style: AppFonts.roboto(fontSize: 18, fontWeight: FontWeight.bold, letterSpacing: 8), decoration: InputDecoration(hintText: '000000', hintStyle: AppFonts.roboto(color: Colors.grey.shade400, letterSpacing: 8), counterText: '', filled: true, fillColor: Colors.grey.shade50, border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.blue.shade500, width: 2)), contentPadding: const EdgeInsets.symmetric(vertical: 16)), onChanged: (value) { if (value.isNotEmpty && !RegExp(r'^[0-9]+$').hasMatch(value)) { _otpController.text = value.replaceAll(RegExp(r'[^0-9]'), ''); _otpController.selection = TextSelection.fromPosition(TextPosition(offset: _otpController.text.length)); } }), const SizedBox(height: 24), SizedBox(height: 48, child: ElevatedButton(onPressed: (_isLoading || _otpController.text.length != 6) ? null : _handleVerifyOtp, style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade600, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), disabledBackgroundColor: Colors.grey.shade300), child: _isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(Colors.white))) : Text('Verify Code', style: AppFonts.roboto(fontSize: 15, fontWeight: FontWeight.w600)))), const SizedBox(height: 16), TextButton(onPressed: _resendCooldown > 0 ? null : _handleResendOtp, child: Text(_resendCooldown > 0 ? 'Resend in ${_resendCooldown}s' : 'Resend Code', style: AppFonts.roboto(fontSize: 14, fontWeight: FontWeight.w600, color: _resendCooldown > 0 ? Colors.grey.shade400 : Colors.blue.shade600)))]);
  Widget _buildStep3() { final strength = _getPasswordStrength(); return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Text('New Password', style: AppFonts.roboto(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade700)), const SizedBox(height: 8), TextField(controller: _newPasswordController, obscureText: !_showPassword, style: AppFonts.roboto(fontSize: 14), decoration: InputDecoration(hintText: 'Enter new password', hintStyle: AppFonts.roboto(color: Colors.grey.shade400), prefixIcon: Icon(Icons.lock_rounded, color: Colors.grey.shade400), suffixIcon: IconButton(icon: Icon(_showPassword ? Icons.visibility_off : Icons.visibility, color: Colors.grey.shade400), onPressed: () => setState(() => _showPassword = !_showPassword)), filled: true, fillColor: Colors.grey.shade50, border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.blue.shade500, width: 2)), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12)), onChanged: (_) => setState(() {})), const SizedBox(height: 16), Text('Confirm Password', style: AppFonts.roboto(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade700)), const SizedBox(height: 8), TextField(controller: _confirmPasswordController, obscureText: !_showConfirmPassword, style: AppFonts.roboto(fontSize: 14), decoration: InputDecoration(hintText: 'Confirm new password', hintStyle: AppFonts.roboto(color: Colors.grey.shade400), prefixIcon: Icon(Icons.lock_rounded, color: Colors.grey.shade400), suffixIcon: IconButton(icon: Icon(_showConfirmPassword ? Icons.visibility_off : Icons.visibility, color: Colors.grey.shade400), onPressed: () => setState(() => _showConfirmPassword = !_showConfirmPassword)), filled: true, fillColor: Colors.grey.shade50, border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.grey.shade300)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: Colors.blue.shade500, width: 2)), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12)), onChanged: (_) => setState(() {})), const SizedBox(height: 16), Row(children: [Expanded(child: Container(height: 8, decoration: BoxDecoration(color: Colors.grey.shade200, borderRadius: BorderRadius.circular(4)), child: FractionallySizedBox(alignment: Alignment.centerLeft, widthFactor: strength == 'weak' ? 0.33 : strength == 'medium' ? 0.66 : strength == 'strong' ? 1.0 : 0.0, child: Container(decoration: BoxDecoration(color: strength == 'weak' ? Colors.red.shade500 : strength == 'medium' ? Colors.yellow.shade600 : strength == 'strong' ? Colors.green.shade500 : Colors.transparent, borderRadius: BorderRadius.circular(4)))))), const SizedBox(width: 12), Text(strength ?? 'Enter password', style: AppFonts.roboto(fontSize: 12, fontWeight: FontWeight.w600, color: strength == 'weak' ? Colors.red.shade600 : strength == 'medium' ? Colors.yellow.shade700 : strength == 'strong' ? Colors.green.shade600 : Colors.grey.shade400))]), const SizedBox(height: 12), _buildPasswordRequirement('At least 8 characters', _newPasswordController.text.length >= 8), _buildPasswordRequirement('One uppercase letter', RegExp(r'[A-Z]').hasMatch(_newPasswordController.text)), _buildPasswordRequirement('One number', RegExp(r'[0-9]').hasMatch(_newPasswordController.text)), const SizedBox(height: 24), SizedBox(height: 48, child: ElevatedButton(onPressed: (_isLoading || !_isPasswordValid) ? null : _handleResetPassword, style: ElevatedButton.styleFrom(backgroundColor: Colors.blue.shade600, foregroundColor: Colors.white, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), disabledBackgroundColor: Colors.grey.shade300), child: _isLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(Colors.white))) : Text('Reset Password', style: AppFonts.roboto(fontSize: 15, fontWeight: FontWeight.w600))))]); }
  Widget _buildPasswordRequirement(String text, bool isMet) => Padding(padding: const EdgeInsets.only(bottom: 4), child: Row(children: [Icon(Icons.check, size: 16, color: isMet ? Colors.green.shade600 : Colors.grey.shade400), const SizedBox(width: 8), Text(text, style: AppFonts.roboto(fontSize: 12, color: isMet ? Colors.green.shade700 : Colors.grey.shade500))]));
  Widget _buildNavigation() => Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [if (_currentStep > 1) TextButton.icon(onPressed: _goToPreviousStep, icon: const Icon(Icons.arrow_back, size: 18), label: Text('Back', style: AppFonts.roboto(fontSize: 14, fontWeight: FontWeight.w600)), style: TextButton.styleFrom(foregroundColor: Colors.grey.shade700)) else const SizedBox.shrink(), TextButton(onPressed: () => Navigator.of(context).pushReplacementNamed('/login'), child: Text('Back to Login', style: AppFonts.roboto(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.blue.shade600)))]));
}
