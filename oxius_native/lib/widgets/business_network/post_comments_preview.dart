...
class _PostCommentsPreviewState extends State<PostCommentsPreview> {
  ...
  Widget build(BuildContext context) {
    ...
    final replies = widget.post.comments
        .where((c) =>
    ...
    if (highestGiftComment != null) {
      displayedComments = displayedComments
          .where((c) => c.id != highestGiftComment!.id)
          .toList();
    }
    ...
    if (widget.showAll) {
      ...
      recentParents = displayedComments.length <= 3
          ...
          : displayedComments
              .sublist(displayedComments.length - 3)
              .reversed
              .toList();
    }
        ...
        children: [
          ...
          ...recentParents.asMap().entries.expand((entry) {
            ...
            final commentReplies = replies.where((r) {
              ...
              ..sort(
                  (a, b) => a.createdAt.compareTo(b.createdAt)); // Sort by time
            ...
            return [
              ...
              if (repliesToShow.isNotEmpty) ...[
                ...
                if (widget.showAll) ...[
                  if (commentReplies.length > 4 && !isExpanded)
                    Padding(
                      padding:
                          const EdgeInsets.only(left: 48, bottom: 8, right: 8),
                      child: Row(
              ...
              if (index < recentParents.length - 1) const SizedBox(height: 12),
            ];
          }),
...
class _CommentItemState extends State<_CommentItem> {
  ...
  void initState() {
    ...
    final plainContent =
        HtmlContentUtils.toPlainText(widget.comment.content).replaceAllMapped(
      RegExp(r'@([^@]+?)  '),
      (match) => '@${match.group(1)} ',
    );
          ...
          children: [
            ...
            Padding(
              ...
              child: GestureDetector(
                onTap: () {
                  final userId = widget.comment.user.uuid ??
                ...
                child: Container(
                  ...
                  child: ClipOval(
                    child: () {
                      final rawAvatarUrl = widget.comment.user.image ??
                          ...
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: () async {
                                ...
                                final controller =
                                    _editMentionKey.currentState?.controller;
                                final plainText = controller?.text ?? '';
                                final markup = controller?.markupText ?? '';
                                final formattedText =
                                    MentionParser.markupToDelimitedText(markup)
                                        .trim();

                                final updatedComment =
                                    await BusinessNetworkService.updateComment(
                                  commentId: widget.comment.id,
                                  content: formattedText.isNotEmpty
                                      ? formattedText
                                      : plainText.trim(),
                                );
                                if (!mounted) return;
                    ...
                    Column(
                      ...
                      children: [
                        Text.rich(
                          TextSpan(
                            children:
                                MentionParser.parseTextWithMentionsAndLinks(
                              HtmlContentUtils.toPlainText(
                                  widget.comment.content),
                              context,
                              style: TextStyle(
