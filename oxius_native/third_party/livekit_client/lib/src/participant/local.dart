...
class LocalParticipant extends Participant<LocalTrackPublication> {
  ...
  Future<LocalTrackPublication<LocalVideoTrack>> publishVideoTrack(
    ...
    Future<lk_models.TrackInfo> negotiate() async {
      // VENDORED PATCH (oxius). `publishOptions` is a nullable capture from the
      // enclosing scope. Dart does not keep a local promoted across an `await`,
      // so every `publishOptions.x` below the first line is an error on newer
      // SDKs ("could not be promoted due to an 'await'"). Binding it once here
      // is exactly equivalent — the original line already asserted non-null
      // with `publishOptions!` — and is what upstream does in later versions.
      final opts = publishOptions!;

      track.transceiver = await room.engine.createTransceiverRTCRtpSender(track, opts, encodings);
      ...
      if ([TrackSource.camera, TrackSource.screenShareVideo].contains(track.source)) {
        final degradationPreference = opts.degradationPreference ??
      ...
      if (kIsWeb && lkBrowser() == BrowserType.firefox && track.kind == TrackType.AUDIO) {
        //TOOD:
      } else if (isSVCCodec(opts.videoCodec) && encodings?.first.maxBitrate != null) {
