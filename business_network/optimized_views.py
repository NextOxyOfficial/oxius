...
class OptimizedBusinessNetworkPostListView(generics.ListAPIView):
    ...
    permission_classes = [
        AllowAny
    ]  # Changed for testing, use IsAuthenticated in production
...
class CachedBusinessNetworkStatsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            if not cached_stats:
                cached_stats = {
                    "following_count": BusinessNetworkFollowerModel.objects.filter(
                        follower=user
                    ).count(),
                    "followers_count": BusinessNetworkFollowerModel.objects.filter(
                        following=user
                    ).count(),
                    "posts_count": BusinessNetworkPost.objects.filter(
                        author=user
                    ).count(),
                    "total_likes_received": BusinessNetworkPostLike.objects.filter(
                        post__author=user
                    ).count(),
                }
