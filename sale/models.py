...
class SaleCategory(models.Model):
    ...
    icon = models.ImageField(
        upload_to='sale_categories/', null=True, blank=True)
...
class SaleChildCategory(models.Model):
    ...
    parent = models.ForeignKey(
        SaleCategory, on_delete=models.CASCADE, related_name='child_categories')
    name = models.CharField(max_length=255)
    icon = models.ImageField(
        upload_to='sale_child_categories/', null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class SaleBanner(models.Model):
    ...
    category = models.ForeignKey(
        SaleCategory, on_delete=models.SET_NULL, null=True, blank=True, related_name='banners')
...
class SaleCondition(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text="Display name (e.g., 'Brand New')")
    value = models.CharField(max_length=100, unique=True,
                             help_text="Value for database (e.g., 'brand-new')")
    description = models.TextField(
        blank=True, null=True, help_text="Optional description of this condition")
    order = models.PositiveIntegerField(
        default=0, help_text="Display order in the dropdown")
    is_active = models.BooleanField(
        default=True, help_text="Whether this condition is currently available for selection")
    created_at = models.DateTimeField(auto_now_add=True)
...
class SalePost(models.Model):
    ...
    category = models.ForeignKey(
        SaleCategory, on_delete=models.SET_NULL, null=True, related_name='posts')
    child_category = models.ForeignKey(
        SaleChildCategory, on_delete=models.SET_NULL, null=True, blank=True, related_name='posts')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.condition and not self.condition_object:
            try:
                condition_obj = SaleCondition.objects.filter(
                    value=self.condition).first()
                if condition_obj:
...
class SaleImage(models.Model):
    ...
    post = models.ForeignKey(
        SalePost, on_delete=models.CASCADE, related_name='images')
