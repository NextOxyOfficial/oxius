...
class SaleChildCategorySerializer(serializers.ModelSerializer):
    parent_category = SaleCategorySerializerParent(
...
class SaleBannerSerializer(serializers.ModelSerializer):
    category_details = SaleCategorySerializer(
...
class SalePostDetailSerializer(serializers.ModelSerializer):
    ...
    category_details = SaleCategorySerializer(
        source='category', read_only=True)
    child_category_details = SaleChildCategorySerializer(
        source='child_category', read_only=True)
    ...
    class Meta:
        model = SalePost
        fields = [
            'id', 'title', 'slug', 'description', 'condition', 'condition_details',
...
class SalePostCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if condition_value:
            try:
                condition_object = SaleCondition.objects.filter(
                    value=condition_value).first()
                if condition_object:
                    validated_data['condition_object'] = condition_object
                    logger.info(f"Found condition object: {condition_object}")
                else:
                    logger.warning(
                        f"No condition object found for value: {condition_value}")
