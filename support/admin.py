...
class SupportTicketAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Ticket Information", {"fields": ("user", "title", "message", "status")}),
        ("Message Preview", {"fields": ("message_preview",), "classes": ("collapse",)}),
        (
            "Metadata",
    ...
    def is_unread_for_user(self, obj):
        ...
        try:
            ...
            if is_unread:
                return format_html(
                    '<span style="color: red; font-weight: bold;">●</span> Unread'
                )
            else:
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not obj.is_processed:
            try:
                ...
                messages.error(request, f"Error processing bulk ticket: {str(e)}")
...
@admin.register(PublicContact)
class PublicContactAdmin(admin.ModelAdmin):
    list_display = ("id", "name", "email", "phone", "status", "created_at")
    list_filter = ("status", "created_at")
    search_fields = ("name", "email", "phone", "message")
    readonly_fields = ("id", "created_at", "updated_at", "message_preview")
    list_editable = ("status",)

    fieldsets = (
        ("Contact Information", {"fields": ("name", "email", "phone", "status")}),
        (
            "Message",
            {
        ...
        (
            "Admin Fields",
            {
                "fields": ("admin_notes", "responded_by", "responded_at"),
                "classes": ("collapse",),
            },
        ),
        (
    ...
    def message_preview(self, obj):
        ...
        if obj.message:
            return format_html(
                '<div style="max-height: 200px; overflow-y: auto; border: 1px solid #ddd; padding: 10px; background: #f9f9f9; white-space: pre-wrap;">{}</div>',
                obj.message,
            )
        return "No message"
